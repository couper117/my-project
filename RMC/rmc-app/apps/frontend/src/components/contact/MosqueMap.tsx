...
import { Navigation, X, Users, Clock, Phone, Car, Footprints } from 'lucide-react';
import { Avatar } from '@/components/ui/Avatar';
import 'leaflet/dist/leaflet.css';

export interface MapMosque {
  id: string;
  name: string;
  address: string | null;
...
interface RouteInfo { km: number; min: number | null }
/** Driving and walking routes for the same destination, shown side by side. */
interface DualRoute { car: RouteInfo; foot: RouteInfo }

/** Map a GeoJSON province `shapeName` to the backend province `code`. */
...
/* Green map pin as a divIcon — avoids the broken-image issue Leaflet's default
   PNG markers hit under bundlers (the icon URLs don't resolve through webpack). */
const pinIcon = L.divIcon({
  className: 'rmc-mosque-pin',
...
/* Amber pin for the mosque nearest the user — larger, with a glow so it stands
   out from the green pins. */
const nearestIcon = L.divIcon({
  className: 'rmc-mosque-pin rmc-mosque-pin--nearest',
...
/** Fly to a focused mosque (e.g. picked from the list or search). */
function FlyToMosque({ focus }: { focus: { lat: number; lng: number; nonce: number } | null }) {
  const map = useMap();
  useEffect(() => {
...
/**
 * Zoom to the mosque nearest the user once it's resolved — but only on the first
...
/**
 * Draws a route from the user's GPS position to the target mosque. Uses the
...
function DirectionsLayer({
  ...
  onInfo: (info: DualRoute | null) => void;
}) {
  const map = useMap();
  const [user, setUser] = useState<[number, number] | null>(null);
  const [carLine, setCarLine] = useState<[number, number][] | null>(null);
  const [footLine, setFootLine] = useState<[number, number][] | null>(null);
  ...
  useEffect(() => {
    if (!target || target.mosque.gpsLat == null || target.mosque.gpsLng == null) {
      setUser(null);
      setCarLine(null);
      setFootLine(null);
    ...
    const dest: [number, number] = [target.mosque.gpsLat, target.mosque.gpsLng];
    let cancelled = false;
    ...
    // Fetch one ORS route for the given travel profile (driving-car / foot-walking).
    const fetchRoute = async (origin: [number, number], profile: string) => {
      const url =
        `https://api.openrouteservice.org/v2/directions/${profile}?api_key=${ORS_KEY}` +
      ...
      async (pos) => {
        ...
        try {
          ...
          const [car, foot] = await Promise.all([
            fetchRoute(origin, 'driving-car'),
            fetchRoute(origin, 'foot-walking'),
          ]);
          if (cancelled) return;
          setCarLine(car.coords);
          setFootLine(foot.coords);
          onInfoRef.current({ car: { km: car.km, min: car.min }, foot: { km: foot.km, min: foot.min } });
          map.fitBounds(L.latLngBounds([...car.coords, ...foot.coords]), { padding: [40, 40] });
          onStatusRef.current('idle');
        } catch {
          if (cancelled) return;
          // Straight-line fallback (no key / API error): one line, estimated times
          // from typical driving (~40 km/h) and walking (~5 km/h) speeds.
          const km = haversineKm(origin, dest);
          const coords: [number, number][] = [origin, dest];
          setCarLine(coords);
          setFootLine(null);
          onInfoRef.current({
            car: { km, min: Math.round((km / 40) * 60) },
            foot: { km, min: Math.round((km / 5) * 60) },
          });
  ...
  return (
    <>
      ...
      {carLine && (
        <Polyline positions={carLine} pathOptions={{ color: '#2563eb', weight: 4, opacity: 0.85 }} />
      )}
      {footLine && (
        <Polyline positions={footLine} pathOptions={{ color: '#1F8A4C', weight: 3, opacity: 0.9, dashArray: '6 8' }} />
      )}
    </>
  );
}
...
interface Props {
  ...
  onRequestDirections: (m: MapMosque) => void;
...
export default function MosqueMap({
  ...
  const [info, setInfo] = useState<DualRoute | null>(null);
  const [userPos, setUserPos] = useState<[number, number] | null>(null);

  // Ask for the user's location once on mount so we can highlight the nearest
  // mosque. Failure (denied / unsupported) just leaves every pin green.
  useEffect(() => {
  ...
  // The mosque closest to the user, among those currently shown.
  const nearest = useMemo(() => {
    if (!userPos) return null;
    let best: { id: string; km: number; pos: [number, number] } | null = null;
    for (const m of mosques) {
      if (m.gpsLat == null || m.gpsLng == null) continue;
      const km = haversineKm(userPos, [m.gpsLat, m.gpsLng]);
      if (!best || km < best.km) best = { id: m.id, km, pos: [m.gpsLat, m.gpsLng] };
    }
    return best;
  }, [userPos, mosques]);
    ...
    <div className="relative h-full w-full">
      ...
      >
        ...
        {mosques
          ...
          .map((m) => {
            const isNearest = m.id === nearestId;
            return (
            <Marker
              key={m.id}
              position={[m.gpsLat as number, m.gpsLng as number]}
              icon={isNearest ? nearestIcon : pinIcon}
            ...
            >
              <Popup>
                ...
                {m.address && <span className="block text-gray-500 text-xs mt-0.5">{m.address}</span>}

                {/* Mosque details */}
                {(m.capacity != null || m.fridayPrayerTime || m.phone) && (
                  <span className="mt-1.5 flex flex-col gap-1 text-[11px] text-gray-600">
                    {m.capacity != null && (
                      <span className="flex items-center gap-1.5">
                        <Users className="w-3 h-3 shrink-0 text-gray-400" />
                        {capacityLabel}: <span className="font-medium text-gray-800">{m.capacity.toLocaleString()}</span>
                      </span>
                    )}
                    {m.fridayPrayerTime && (
                      <span className="flex items-center gap-1.5">
                        <Clock className="w-3 h-3 shrink-0 text-gray-400" />
                        {jumuahLabel}: <span className="font-medium text-gray-800">{m.fridayPrayerTime.slice(0, 5)}</span>
                      </span>
                    )}
                    {m.phone && (
                      <a href={`tel:${m.phone.replace(/\s+/g, '')}`} className="flex items-center gap-1.5 hover:underline">
                        <Phone className="w-3 h-3 shrink-0 text-gray-400" />
                        <span className="font-medium text-gray-800">{m.phone}</span>
                      </a>
                    )}
                  </span>
                )}
                ...
                {m.imamName && (
                  <span className="mt-1.5 flex items-center gap-2">
                    <Avatar name={m.imamName} photo={m.imamPhoto} size={30} />
                    <span className="leading-tight">
                      <span className="block text-[10px] uppercase tracking-wide text-gray-400">{imamLabel}</span>
                      <span className="block text-xs font-medium text-gray-800">{m.imamName}</span>
                      {m.imamPhone && (
                        <a
                          href={`tel:${m.imamPhone.replace(/\s+/g, '')}`}
                          className="mt-0.5 inline-flex items-center gap-1 text-[11px] text-rmc-green hover:underline"
                        >
                          <Phone className="w-2.5 h-2.5 shrink-0" /> {m.imamPhone}
                        </a>
                      )}
                    </span>
                  </span>
      ...
      {directionsTarget && (
        <div className="absolute bottom-3 left-1/2 z-[1000] flex -translate-x-1/2 items-center gap-2.5 rounded-full border border-gray-100 bg-white/95 px-3.5 py-1.5 text-xs shadow-lg ring-1 ring-black/5 backdrop-blur">
          {status === 'locating' || status === 'routing' ? (
            ...
            <span className="flex flex-wrap items-center gap-x-2.5 gap-y-1 font-medium text-gray-800">
              <span className="flex items-center gap-1">
                <Car className="h-3.5 w-3.5 text-rmc-green" />
                {info.car.km.toFixed(1)} km{info.car.min != null ? ` · ${info.car.min} min` : ''}
              </span>
              <span className="flex items-center gap-1 border-l border-gray-200 pl-2.5">
                <Footprints className="h-3.5 w-3.5 text-rmc-green" />
                {info.foot.km.toFixed(1)} km{info.foot.min != null ? ` · ${info.foot.min} min` : ''}
              </span>
            </span>
          ) : null}
          <button
